...
void test4()
{
    ...
    if (ioctl(device0_fd, MSG_SLOT_CHANNEL, 115) < 0)
    { print_failure(4); exit(0); }
...
void test5()
{
        ...
        device0_fd = open(DEV0, O_RDWR);
        if (device0_fd < 0)
        { print_failure(5); perror("#1"); exit(0); }

        if (ioctl(device0_fd, MSG_SLOT_CHANNEL, 6) < 0)
        { print_failure(5); perror("#2");exit(0); }

	device1_fd = open(DEV1, O_RDWR);
	if (device1_fd < 0)
	{ print_failure(5); perror("#3"); exit(0); }

        if (write(device1_fd, "hey", 3) >= 0)
        { print_failure(5); perror("#4"); exit(0); }
...
void test12()
{
    ...
    device0_fd = open(DEV0, O_RDWR);
        ...
        { printf("BYTES READ: %d, ERRNO %d != EINVAL %d, EWOULDBLOCK %d, ENOSPC %d", (int) bytes_read, errno, EINVAL, EWOULDBLOCK, ENOSPC);
            print_failure(12); exit(0); }
